...
Last activity: 2026-09-10 -- Quick task 260910-hrj: v2.7.1 **prepared, not published** (metadata 2.7.1/12 + changelog; deliberately untagged, unpushed -- tagging fires the release workflow); packed and installed locally so the wrap fix is live at next login. Two defects found in CLAUDE.md's own release procedure and fixed: the zip entry count was stale (24/~198 KB, not 21/~140 KB), and `gnome-extensions install --force` follows a symlinked dev install and deletes the repo's `usbee@bitcreed.us/` sources -- it did exactly that here (recovered via `git checkout`, nothing uncommitted was lost) and the symlinked dev install is now a static copy
Previous activity: 2026-09-10 -- Quick task 260910-ggy: device property-panel values wrap instead of ellipsizing (St.Label's inherited `PANGO_ELLIPSIZE_END` was cancelling the `line_wrap` the code already set); same fix in `empty-state.js`; CI guard on the pairing
