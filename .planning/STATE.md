...
| 260822-hkn | Document the canonical `gnome-extensions pack` invocation (`--podir` + both `--extra-source` flags) in CLAUDE.md as shared source of truth with release.yml; warn that a wrong `--extra-source` is silently ignored and yields a `src/`-less zip; add local install + on-disk verification steps and the `gnome-extensions info` staleness caveat | 2026-08-22 | 5a7dc33 | [260822-hkn-document-pack-and-local-install](./quick/260822-hkn-document-pack-and-local-install/) |
